...
export default function Button (props) {
  ...
  const party = useRef({})
  const toggle = async () => {
    if (session) setSession(await party.current.disconnect("user"))
    else setSession(await party.current.connect("user", payload))
  }
  useEffect(() => {
    const web3 = new Web3(window.ethereum)
    party.current = new Privateparty({ web3, host });
    (async () => {
